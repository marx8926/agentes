...
to setup-patches 
  ...
  set-default-shape administrators "arrow" ;; Los samurais se mostraran como flechas
  ...
  set-default-shape abilities "plant"
...
to setup-delivers
  ...
  [
    set color blue ;; El color de su equipo
  ...
  [
   ...
   create-delivers 1 ;; Crea el primer castillo, lo pone en una esquina e inicializa sus variables
  [
    set color brown ;; El color de su equipo
    setxy (min-pxcor - 5) (max-pycor - 5) ;; Las coordenadas del castillo
    set hp 10 ;; Los hit-points del castillo
    set team 4 ;; Equipo 1
  ] 
  ...
  ask delivers [if team = 1 [ask patches in-radius 3 [set pcolor blue]] ] 
  ...
  ask delivers [if team = 4 [ask patches in-radius 3 [set pcolor brown]] ] 
...
;;  ask workers with [team = 1] [move-to one-of patches with [(pcolor = 107) and (not any? workers-here)]]  ;; Coloca el trabajador cerca del castillo

 
end
  
to setup-resources
  create-resources 20 ;; Creara la cantidad de recursos especificados e inicializara sus variables
  [
  ...
  ;ask resources [while [(
  ;      ((count patches with [pcolor = 107] in-radius 2 ) > 1)or
  ;      ((count patches with [pcolor = 17] in-radius 2 ) > 1)or
  ;      ((count patches with [pcolor = white] in-radius 2 ) > 1)or
  ;      ((count patches with [pcolor = black] in-radius 2 ) > 1)or
  ;      ((count patches with [pcolor = 106] in-radius 2 ) > 1)or
  ;      ((count patches with [pcolor = 16] in-radius 2 ) > 1)or
  ;      ((count resources in-radius 6) > 1)
  ;      )] [setxy random-xcor random-ycor]] ;; Los distribuye en el mapa, para que no queden encima de un castillo o en sus alrededores
